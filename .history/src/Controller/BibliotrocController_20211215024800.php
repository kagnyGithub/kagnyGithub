...
class BibliotrocController extends AbstractController
{
    ...
    */
    public function showStock(Request $request, EntityManagerInterface $manager, StockerRepository $repo, CategorieRepository $repocategorie){
        $user = $this->getUser();
      ...
      public function addMyStocke(Livres $livre, Request $request, EntityManagerInterface $manager, StockerRepository $repo, CategorieRepository $repocategorie){
          $user = $this->getUser();
          if(!$user){
              return $this->redirectToRoute('security_login');
          }else{
              $stocker=new Stocker();  
              $stocker->setIdUser($user)
                      ->setIdLivre($livre)
                      ->setEnvoyes(false);
      ...
      public function suprimerLivreStocker(Stocker $stocker,  EntityManagerInterface $manager){
